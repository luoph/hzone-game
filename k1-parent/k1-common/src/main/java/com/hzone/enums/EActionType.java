...
public enum EActionType {
    ...
    /** 分割字符串为 map[EActionType, Float]. 字符串格式: k1:v1,k2:v2,k3:v3, */
    public static Map<EActionType, Float> parseToMap(String str) {
        return StringUtil.splitToMap(str, s -> EActionType.convertByName(s), StringUtil.toFloat);
    }

    public static String toString(Map<EActionType, Float> map) {
        if (map == null || map.isEmpty()) {
            return "";
        }
        StringBuilder sb = new StringBuilder();
        for (Map.Entry<EActionType, Float> e : map.entrySet()) {
            sb.append(e.getKey().getConfigName()).append(StringUtil.COLON)
                .append(e.getValue()).append(StringUtil.COMMA);
        }
        return sb.toString();
    }
}
