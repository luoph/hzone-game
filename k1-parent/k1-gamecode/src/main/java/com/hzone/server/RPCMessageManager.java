...
import com.hzone.server.rpc.RPCClientKeepAlive;
...
import com.hzone.server.rpc.handler.RPCHandler;
...
public class RPCMessageManager {
    ...
    private static void addRouteServer0(RPCServer server, RPCServer local, boolean replace) {
        log.info("test zk server is open "+server.isOpen());
        if (!server.isOpen()) {
            return;
        }
    ...
    public static void sendMessage(int code, String node, Serializable... args) {
        ...
        if (route == null) {
            log.error("[无法获取存活的路由节点] getRouteServer is null. code {} node {} args {}", code, node, args);
            return;
        }
