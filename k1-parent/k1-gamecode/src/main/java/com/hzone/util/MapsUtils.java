...
import com.hzone.server.http.bean.FTXLogin;
...
public class MapsUtils {
    ...
    /** 把 src 合并进 target */
    public static <K, V extends Number> void mergeValueInt(Map<K, V> src, Map<K, V> target,
                                                           BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
        if (src == null || src.isEmpty()) {
            return;
        }
        for (Map.Entry<K, V> e : src.entrySet()) {
            target.merge(e.getKey(), e.getValue(), remappingFunction::apply);
        }
    }
