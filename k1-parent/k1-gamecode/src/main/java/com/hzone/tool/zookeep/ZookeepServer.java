package com.hzone.tool.zookeep;
...
public class ZookeepServer {
        ...
        try {
            Watcher watcher = event -> {
                ...
                if (event.getType() == EventType.None) {
                    log.info("ZookeepServer lianjie1");
                    //创建逻辑节点，
                    ...
                    log.info("ZookeepServer lianjie2");
                    List<String> routeList = zk.getChild(ZookeepServer.getRoutePath(), routeWatcher);
                    if(routeList == null) {
                      log.info("ZookeepServer lianjie3 routeList is null");
                    }else {
                      log.info("ZookeepServer lianjie3 routeList size "+routeList.size());
                    }
                    //将新的数据推送到路由服务器
    ...
    public String create(String node, String msg) {
        try {
            ...
            log.info("创建一个zookeep节点[{}],节点信息[{}] ret {}", node, msg, path);
