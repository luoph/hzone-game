...
public class JedisUtil {
    ...
    public <T extends Serializable> void lpush(String key, T obj) {
        try (Jedis jedis = getJedis()) {
            byte[] keybyte = DefaultCodec.encode(key);
            jedis.lpush(keybyte, DefaultCodec.encode(obj));
        } catch (Exception e) {
            log.error("redis lpush. " + e.getMessage(), e);
        }
    }
    ...
    public void ltrim(String key, int start, int stop) {
        try (Jedis jedis = getJedis()) {
            byte[] keybyte = DefaultCodec.encode(key);
            jedis.ltrim(keybyte, start, stop);
        } catch (Exception e) {
            log.error("redis ltrim. " + e.getMessage(), e);
        }
    }

    public <T extends Serializable> List<T> lrange(String key, int start, int stop) {
        try (Jedis jedis = getJedis()) {
            byte[] keybyte = DefaultCodec.encode(key);
            List<byte[]> list = jedis.lrange(keybyte, start, stop);
            if (list == null || list.isEmpty()) {
                return Collections.emptyList();
            }
            List<T> ret = new ArrayList<>(list.size());
            for (byte[] e : list) {
                ret.add(DefaultCodec.decode(e));
            }
            return ret;
        } catch (Exception e) {
            log.error("redis lrange. " + e.getMessage(), e);
        }
        return Collections.emptyList();
    }
